...
from openalea.stse.structures.algo.walled_tissue_topology import \
    test_if_division_is_possible, create_shapes_for_new_cells
...
def dscs_shortest_wall( wt, cell ):
    ...
    if test_if_division_is_possible( wt, shape ):
        dshape = shape+shape
        ls = len(shape)
        vmin = float( "infinity" )
        for i in range( ls ): 
        ...
        raise Exception(" !: Division not possible. Cell degenerated.")
...
def dscs_shortest_wall_with_equal_surface( wt, cell, **keys ):
    ...
    keys:
    * nbr_of_segments_in_wall == 6
    * length_tolerance == 0.1
    """
    if keys.has_key("nbr_of_segments_in_wall"):
        z = keys[ "nbr_of_segments_in_wall" ]
    else:
        z = 6
    if keys.has_key("length_tolerance"):
        length_tolerance = keys[ "length_tolerance" ]
    else:
        length_tolerance = 0.1
    try:
        import pqueue
    except:
        print "!: To use this division strategy please install pqueue  python module (e.g. apt-get install python-pyqueue)"
    surfs = pqueue.PQueue()
    shape = wt.cell2wvs( cell )
    if test_if_division_is_possible( wt, shape ):
        dshape = shape+shape
        ...
        for i in range( ls ): 
            ...
            assert False #TODO: find what was the old function corresponding to next line
                #     next function name is not really correct...
            wvc1, wvc2 = create_shapes_for_new_cells( wt, cell, (s1, t1), (s2, t2) )
            ...
            for k in range( 1,z ):
                ...
                surf1 = walled_tissue.calculate_cell_surfaceS( wvc1, wv2pos ) 
                surf2 = walled_tissue.calculate_cell_surfaceS( wvc2, wv2pos ) 
        ...
        while len( surfs ):
            ...
            if length_tolerance*pgl.norm( p1m - p2m ) < shortest_possible_len: 
        ...
        Exception(" !: Division not possible. Cell degenerated.")
    #return ( (s1m, t1m, p1m), (s2m, t2m, p2m) )
